    ...
    pub fn create<P: AsRef<Path>>(path: P) -> crate::Result<Self> {
            ...
            if let Ok(existing_pid) = Self::read_pid(&path) {
                ...
                if Self::is_process_running(existing_pid) {
                    return Err(crate::Error::Service(
                        format!("Service already running with PID {}", existing_pid)
                    ));
                }
                info!("Removing stale PID file for non-running process {}", existing_pid);
        ...
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent)
                .map_err(|e| crate::Error::Service(format!("Failed to create PID directory: {}", e)))?;
        }
        ...
        #[cfg(unix)]
        {
            ...
            match Flock::lock(file, FlockArg::LockExclusiveNonblock) {
                Ok(flock) => {
                    ...
                    (true, Some(Box::new(flock) as Box<dyn std::any::Any + Send + Sync>))
                }
                Err((_file, e)) => {
