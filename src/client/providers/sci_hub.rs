...
impl SciHubProvider {
    ...
    fn convert_query(&self, query: &SearchQuery) -> String {
        match query.search_type {
            SearchType::Doi => {
                // Clean DOI format
                query.query.trim_start_matches("doi:").trim_start_matches("https://doi.org/").to_string()
            }
            _ => query.query.clone(),
        }
    }
    ...
    fn convert_response(&self, response: ResearchResponse, start_time: Instant, query: &SearchQuery) -> ProviderResult {
        ...
        metadata.insert("source_mirror".to_string(), response.source_mirror.to_string());
        metadata.insert("response_time".to_string(), response.response_time.as_millis().to_string());
...
impl SourceProvider for SciHubProvider {
    ...
    async fn search(&self, query: &SearchQuery, _context: &SearchContext) -> Result<ProviderResult, ProviderError> {
        let start_time = Instant::now();
        
        info!("Searching Sci-Hub for: {} (type: {:?})", query.query, query.search_type);
        ...
        let response = match query.search_type {
            SearchType::Doi => {
                ...
                self.client.search_by_doi(&doi).await
                    .map_err(|e| ProviderError::Other(format!("Sci-Hub DOI search failed: {}", e)))?
            }
            SearchType::Title | SearchType::Auto | SearchType::Keywords => {
                // For title/auto searches, use the title search method
                self.client.search_by_title(&search_term).await
                    .map_err(|e| ProviderError::Other(format!("Sci-Hub title search failed: {}", e)))?
            }
            SearchType::Author => {
                return Err(ProviderError::InvalidQuery("Sci-Hub does not support author search".to_string()));
            }
            SearchType::Subject => {
                return Err(ProviderError::InvalidQuery("Sci-Hub does not support subject search".to_string()));
            }
        };

        let result = self.convert_response(response, start_time, query);
        
        info!("Sci-Hub search completed: {} papers found in {:?}", 
              result.papers.len(), result.search_time);

        Ok(result)
    }

    async fn get_by_doi(&self, doi: &str, _context: &SearchContext) -> Result<Option<PaperMetadata>, ProviderError> {
        info!("Getting paper by DOI from Sci-Hub: {}", doi);

        let doi_obj = Doi::new(doi)
            .map_err(|e| ProviderError::InvalidQuery(format!("Invalid DOI: {}", e)))?;

        let response = self.client.search_by_doi(&doi_obj).await
