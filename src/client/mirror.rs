...
impl Mirror {
    ...
    pub const fn is_usable(&self) -> bool {
        ...
        matches!(self.health, MirrorHealth::Healthy | MirrorHealth::Degraded | MirrorHealth::Unknown)
    }
    ...
    pub fn mark_failure(&mut self) {
        ...
        if self.failure_count >= 3 {
            warn!("Mirror {} marked as unhealthy after {} failures", self.url, self.failure_count);
        }
    }
    ...
    pub fn mark_success(&mut self, response_time: Duration) {
        ...
        debug!("Mirror {} health check: {:?} ({}ms)", 
               self.url, self.health, response_time.as_millis());
    }
    ...
    pub fn needs_health_check(&self) -> bool {
        self.last_check.map_or(true, |last_check| {
                ...
                SystemTime::now()
                    ...
                    .unwrap_or(Duration::ZERO) > check_interval
            })
    }
}
...
impl MirrorManager {
    ...
    pub async fn health_check_all(&self) {
        ...
        info!("Performing health checks on {} mirrors", mirrors_to_check.len());
        
        // Perform health checks concurrently
        let health_check_tasks = mirrors_to_check
            .into_iter()
            .map(|(index, mirror)| {
                ...
                tokio::spawn(async move {
                    ...
                    if let Some(mirror_ref) = mirrors_write.get_mut(index) {
                        match health_check_result {
                            ...
                            Ok(response) => {
                                warn!("Mirror {} health check failed with status: {}", 
                                      mirror_ref.url, response.status());
