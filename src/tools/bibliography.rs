...
impl BibliographyTool {
    ...
    pub async fn generate(&self, input: BibliographyInput) -> Result<BibliographyResult> {
        info!("Generating bibliography for {} papers in {:?} format", 
              input.identifiers.len(), input.format);
    ...
    fn format_citation(
        ...
        let text = match format {
            CitationFormat::BibTeX => self.format_bibtex(metadata, include_abstract, include_keywords),
    ...
    /// Format as BibTeX
    fn format_bibtex(&self, metadata: &PaperMetadata, include_abstract: bool, include_keywords: bool) -> String {
        ...
        if include_keywords && !metadata.keywords.is_empty() {
            parts.push(format!("  keywords = {{{}}},", metadata.keywords.join(", ")));
        }
    ...
    fn generate_bibtex_key(&self, metadata: &PaperMetadata) -> String {
        let first_author = metadata.authors
            .first()
    ...
    fn format_mla(&self, metadata: &PaperMetadata) -> String {
        let authors = if metadata.authors.len() > 1 {
            ...
            metadata.authors.first().cloned().unwrap_or_else(|| "Unknown".to_string())
        };
