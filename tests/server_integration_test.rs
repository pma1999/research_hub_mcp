...
async fn test_server_graceful_shutdown() {
    ...
    assert!(server_result.is_ok(), "Server should complete without errors");
}
...
async fn test_transport_validation() {
    ...
    let result = rust_sci_hub_mcp::server::transport::validate_stdio_transport();
    assert!(result.is_ok(), "Transport validation should allow terminal in development");
}

#[tokio::test]
async fn test_server_handler_integration() {
    let config = Config::default();
    let mut handler = rust_sci_hub_mcp::server::SciHubServerHandler::new(Arc::new(config)).unwrap();
...
async fn test_concurrent_operations() {
    let config = Config::default();
    let handler = Arc::new(rust_sci_hub_mcp::server::SciHubServerHandler::new(Arc::new(config)).unwrap());
